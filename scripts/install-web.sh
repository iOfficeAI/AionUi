...
# VERSION: CI runs `sed "s/__VERSION__/$VERSION/g"` on this file before release.
# To avoid sed replacing our placeholder marker, we use a separate variable
# (_VERSION_WAS_SET) to track whether VERSION was explicitly provided by the
# user, and only resolve from GitHub API when VERSION is "latest" or when the
# default placeholder (__VERSION__) is still present (not a real semver).
_VERSION_WAS_SET="${VERSION+set}"
VERSION="${VERSION:-__VERSION__}"
# Note: CI runs `sed "s/__VERSION__/<ver>/g"` on this file, replacing the
# default value on line 16. The resolve_version() function compares against
# $VERSION_PLACEHOLDER (a bash variable, immune to sed), so versions like
...
resolve_version() {
    # Resolve from GitHub API when:
    # - VERSION is "latest" (explicit)
    # - VERSION was NOT explicitly set AND is not a valid semver (unreplaced
    #   __VERSION__ placeholder from the raw script)
    # Otherwise use VERSION as-is (explicit user value or CI-substituted default).
    ...
    elif [[ -z "$_VERSION_WAS_SET" ]] && ! [[ "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+ ]]; then
        # VERSION came from the default placeholder and wasn't substituted by CI
        info "Using version: ${BOLD}v${VERSION}${NC}"
    else

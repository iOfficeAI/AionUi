...
# Create yml files with version substituted directly (avoids sed -i incompatibility)
write_yml() {
  local dir="$1" file="$2"
  shift 2
  # Write heredoc with variable substitution
  cat > "$dir/$file" <<<"$(printf '%s\n' "$@" | sed "s/VERSION_PLACEHOLDER/${VERSION}/g")"
}

# Windows x64
touch "$ARTIFACTS_DIR/windows-build-x64/POUNDING-${VERSION}-win-x64.exe"
write_yml "$ARTIFACTS_DIR/windows-build-x64" "latest.yml" \

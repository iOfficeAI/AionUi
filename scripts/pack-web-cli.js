...
const tarballName = `csbu-workmate-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'csbu-workmate-web');
...
const executableName = platform === 'win32' ? 'csbu-workmate-web.exe' : 'csbu-workmate-web';
...
// `csbu-workmate-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} csbu-workmate-web`, {

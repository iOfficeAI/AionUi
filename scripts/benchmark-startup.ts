 ...
 * parsing the electron-log file for [CSBU WorkMate:ready] / [CSBU WorkMate:init] /
 * [CSBU WorkMate:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [CSBU WorkMate:ready] marks
  ...
  // Parsed from [CSBU WorkMate:init] marks
  initTotalMs: number;
  // Parsed from [CSBU WorkMate:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'CSBU-WorkMate-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'CSBU WorkMate', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'CSBU WorkMate', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'CSBU WorkMate', 'logs', `${today}.log`));
  }
...
// Matches: [CSBU WorkMate:ready] <label> +<ms>ms
// Matches: [CSBU WorkMate:init]  <label> +<ms>ms
// Matches: [CSBU WorkMate:process] <label> +<ms>ms
const MARK_REGEX = /\[CSBU WorkMate:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[CSBU WorkMate] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[CSBU WorkMate] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[CSBU WorkMate] Showing main window')) marks.logs.showingMainWindow = true;
  }

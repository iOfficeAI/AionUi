 ...
 *
 * Env vars:
 *   CSBU_WORKMATE_PORT           : static server port (default 33000)
 *   CSBU_WORKMATE_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   CSBU_WORKMATE_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   CSBU_WORKMATE_DATA_DIR       : override userData path (default Electron-compatible)
 *   CSBU_WORKMATE_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   CSBU_WORKMATE_STATIC_DIR     : override static dir (default out/renderer)
 *   CSBU_WORKMATE_BACKEND_BIN    : absolute path to aioncore binary (else PATH lookup)
 *   CSBU_WORKMATE_BACKEND_BUNDLED_DIR : dir containing bundled-aioncore/<plat-arch>/binary
 *   CSBU_WORKMATE_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
 ...
 * work on hosts that never installed CSBU WorkMate.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $CSBU_WORKMATE_DATA_DIR        env override (same effect)
 *   otherwise               ~/.csbu-workmate-web         (production)
 *                           ~/.csbu-workmate-web-dev     (dev, default)
 *                           ~/.csbu-workmate-web-dev-2   (dev + CSBU_WORKMATE_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.csbu-workmate[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.csbu-workmate-dev` as a **symlink** to
 * `~/Library/Application Support/CSBU-WorkMate-Dev/csbu-workmate` so CLI tools (claude,
 ...
 * `.csbu-workmate-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.csbu-workmate-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.CSBU_WORKMATE_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.CSBU_WORKMATE_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.csbu-workmate-web${suffix}`);
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.CSBU_WORKMATE_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.CSBU_WORKMATE_ALLOW_REMOTE ?? process.env.CSBU_WORKMATE_REMOTE);
}
 ...
 *   $CSBU_WORKMATE_NO_BUILD=1        : env-level opt-out
 *   $CSBU_WORKMATE_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.CSBU_WORKMATE_NO_BUILD)) return;
  if (process.env.CSBU_WORKMATE_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set CSBU_WORKMATE_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without CSBU WorkMate.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.CSBU_WORKMATE_LOG_DIR ?? path.join(workDir, 'logs');

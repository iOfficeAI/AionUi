...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'csbu-workmate-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'csbu-workmate-installer-report.json'
...
function Read-CsbuWorkMateAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'CSBU WorkMate\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'CSBU WorkMate\analytics.json')
  }
...
function Show-ReportMessage([string]$text, [string]$icon) {
  ...
  [System.Windows.Forms.MessageBox]::Show(
    $text,
    'CSBU WorkMate installer report',
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('csbu-workmate-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('csbu-workmate-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('CSBU WorkMate installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
  ...
  $lines.Add('---------------------------')
  $lines.Add('To CSBU WorkMate Team')
  $lines.Add('---------------------------')
...
try {
  ...
  $userId = Read-CsbuWorkMateAnalyticsId
  $eventPayload = @{
    message = ('installer-failure ' + $Code)
    level = 'error'
    platform = 'other'
    release = $Release
    logger = 'csbu-workmate.installer'
  ...
  Show-ReportMessage ('CSBU WorkMate installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Press [ Ctrl + C ] in this dialog to copy the details, then send them to your internal support contact.' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('CSBU WorkMate installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}

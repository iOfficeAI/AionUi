...
with N.cell("Setup: Calculate factor z-scores"):
...
with N.cell("Observation: Quality + Value combination (unusual)"):
...
with N.cell("Investigation: Why does high-quality stock trade cheap?"):
    ...
    **Testing hypothesis #1**: Quality improvement not priced in
    ...
    N.table(quality_trend, name="Quality vs Valuation Trend")
...
with N.cell("Cross-sectional validation: Is VCB unique?"):
    ...
    N.table(peers_factors, name="Peer Factor Comparison")
...
with N.cell("Macro alignment check: Does weak momentum matter?"):
    # VCB has weak momentum (-0.3) in EXPANSION regime
    # EXPANSION favors momentum/growth, not value
    # Should we care about this misalignment?
...
with N.cell("Catalyst timeline: When will mispricing correct?"):
    # When will market recognize VCB's quality improvement?
...
with N.cell("Bottom line: Factor-based investment thesis"):
    N.kv({

...
所以本期把这个能力**升级为按 Agent 分别检测**——**沿用现有的心跳按钮作为入口，不新增按钮**：

- 点现有的心跳按钮，弹出「针对哪些 Agent 测」的选择（可单选、可多选一起测），结果**按 Agent 分别记录**。
...
**需求**：把这个检测**扩展为按 Agent 分别检测**——**入口就是模型行上现有的心跳按钮，不新增按钮**；检测通过的 Agent，在该模型旁以 **Agent 图标**点亮，让用户直观看到「哪些 Agent 能用这个模型」。

- 点现有的心跳按钮，弹出「针对哪些 Agent 测」的选择：
  - 勾选**一个** Agent → 只测它；
  - 勾选**多个** Agent → 一起测（多选即等于批量检测，不再单独做一个批量按钮）。

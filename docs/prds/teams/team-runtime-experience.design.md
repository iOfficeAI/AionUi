...
export const TEAM_MEMBER_PALETTE = [
  ...
  '#5c9ea4', '#b58a5e', '#9481bf', '#c07d97', '#6ba07e', '#4f8ac9', '#c99a4b',
] as const;
...
export function assignMemberColors(
  ...
  if (leader) { next[leader.slot_id] = LEADER_COLOR_INDEX; used.add(LEADER_COLOR_INDEX); }
  // 2) 已有映射的沿用（钉死）
  for (const a of assistants) {
    if (a.slot_id in next) continue;
    const c = prev[a.slot_id];
    if (c !== undefined) { next[a.slot_id] = c; used.add(c); }
  }
  ...
  for (const a of assistants) {
    ...
    next[a.slot_id] = idx; used.add(idx); cursor++;
  }
  return next;
}
```
...
```
文案 `team.addMember.tellLeaderPrefill` = 「帮我在团队里加一个擅长 ___ 的成员」。

# CSBU WorkMate WebUI Mode - Startup Guide

CSBU WorkMate supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts CSBU WorkMate with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use CSBU WorkMate from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\CSBU WorkMate\CSBU WorkMate.exe" --webui

# Or if CSBU WorkMate is in your PATH
CSBU WorkMate.exe --webui
```
...
2. Enter target location:
   ...
   "C:\Program Files\CSBU WorkMate\CSBU WorkMate.exe" --webui
   ```
3. Name it **CSBU WorkMate WebUI**
4. Click **Finish**
5. Double-click the shortcut to launch

### Method 3: Create a Batch File

Create `start-csbu-workmate-webui.bat`:
...
```bash
# Using full path
/Applications/CSBU WorkMate.app/Contents/MacOS/CSBU WorkMate --webui

# Or using open command
open -a CSBU WorkMate --args --webui
```

### Method 2: Create Shell Script

Create `start-csbu-workmate-webui.sh`:
...
```bash
chmod +x start-csbu-workmate-webui.sh
./start-csbu-workmate-webui.sh
```
...
# Or using full path
/opt/CSBU WorkMate/csbu-workmate --webui
```
...
chmod +x CSBU-WorkMate-*.AppImage

# Run with --webui flag
./CSBU-WorkMate-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/csbu-workmate-webui.desktop`:

```ini
[Desktop Entry]
Name=CSBU WorkMate WebUI
Comment=Start CSBU WorkMate in WebUI mode
Exec=/opt/CSBU WorkMate/csbu-workmate --webui
Icon=csbu-workmate
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/csbu-workmate-webui.desktop
```
...
Create `~/bin/start-csbu-workmate-webui.sh`:
...
```bash
chmod +x ~/bin/start-csbu-workmate-webui.sh
```
...
Create `/etc/systemd/system/csbu-workmate-webui.service`:

```ini
[Unit]
Description=CSBU WorkMate WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/CSBU WorkMate/csbu-workmate --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable csbu-workmate-webui.service
sudo systemctl start csbu-workmate-webui.service

# Check status
sudo systemctl status csbu-workmate-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run CSBU WorkMate in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making CSBU WorkMate accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running CSBU WorkMate WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/CSBU/CSBU-WorkMate/issues/217)
...
#### 4. Download and Install CSBU WorkMate

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/CSBU/CSBU-WorkMate/releases
wget https://github.com/CSBU/CSBU-WorkMate/releases/download/vVERSION/CsbuWorkMate_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/CSBU/CSBU-WorkMate/releases/download/vVERSION/CsbuWorkMate_VERSION_arm64.deb

# Install the package
apt install -y ./CsbuWorkMate_*.deb

# Verify installation
which CSBU WorkMate
```

#### 5. Launch CSBU WorkMate WebUI

```bash
# Start CSBU WorkMate in WebUI mode with no-sandbox flag
CSBU WorkMate --no-sandbox --webui
```
...
To access CSBU WorkMate from other devices on your local network:

```bash
# Start with --remote flag
CSBU WorkMate --no-sandbox --webui --remote
...
CSBU WorkMate --no-sandbox --webui --port 8080
```
...
chmod +x /opt/CSBU WorkMate/csbu-workmate
```

#### Out of Memory

CSBU WorkMate requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if CSBU WorkMate is running: look for "Server started" message
...
4. **Keep device charged** - Running CSBU WorkMate consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **CSBU WorkMate Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch CSBU WorkMate quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-csbu-workmate.sh << 'EOF'
#!/bin/bash
echo "Starting CSBU WorkMate WebUI..."
CSBU WorkMate --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-csbu-workmate.sh

# Run anytime
./start-csbu-workmate.sh
```
...
```bash
proot-distro login ubuntu -- bash -c "CSBU WorkMate --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/CSBU/CSBU-WorkMate/issues/217)
...
```cmd
CSBU WorkMate.exe --webui --remote
```

### macOS

```bash
/Applications/CSBU WorkMate.app/Contents/MacOS/CSBU WorkMate --webui --remote
```

### Linux

```bash
csbu-workmate --webui --remote
```
...
netsh advfirewall firewall add rule name="CSBU WorkMate WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add CSBU WorkMate
...
```cmd
where CSBU WorkMate.exe
```

**macOS:**

```bash
mdfind -name "CSBU WorkMate.app"
```
...
which csbu-workmate
# or
find /opt -name "csbu-workmate" 2>/dev/null
```
...
```powershell
& "C:\Program Files\CSBU WorkMate\CSBU WorkMate.exe" --webui 2>&1 | Tee-Object -FilePath csbu-workmate.log
```

**macOS/Linux:**

```bash
/path/to/csbu-workmate --webui 2>&1 | tee csbu-workmate.log
```
...
export CSBU_WORKMATE_PORT=8080

# Allow remote access without passing --remote
export CSBU_WORKMATE_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as CSBU_WORKMATE_ALLOW_REMOTE=true)
export CSBU_WORKMATE_HOST=0.0.0.0

# Then start the application
csbu-workmate --webui

# You can also pass the port directly via CLI
csbu-workmate --webui --port 8080
```
...
```cmd
# Using full path
"C:\Program Files\CSBU WorkMate\CSBU WorkMate.exe" --resetpass

# Or for a specific user
"C:\Program Files\CSBU WorkMate\CSBU WorkMate.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/CSBU WorkMate.app/Contents/MacOS/CSBU WorkMate --resetpass

# Or for a specific user
/Applications/CSBU WorkMate.app/Contents/MacOS/CSBU WorkMate --resetpass username
```
...
```bash
# Using system path
csbu-workmate --resetpass

# Or for a specific user
csbu-workmate --resetpass username

# Or using full path
/opt/CSBU WorkMate/csbu-workmate --resetpass
```
...
- [GitHub Issues](https://github.com/CSBU/CSBU-WorkMate/issues)
...
2. Search [existing issues](https://github.com/CSBU/CSBU-WorkMate/issues)
3. Create a [new issue](https://github.com/CSBU/CSBU-WorkMate/issues/new) with:
   - Your OS and version
   - CSBU WorkMate version
   - Steps to reproduce
   - Error messages or logs

---

**Happy using CSBU WorkMate in WebUI mode!** 🚀

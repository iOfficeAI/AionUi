...
> 本档案只做调研与归档，不含任何产品代码改动。代码清点/适配/扩展包方案见上级目录（`../README.md`、`../01-原版二开资产清单.md`、`../02-v2.1.52适配矩阵.md`、`../03-扩展包与adapter决策.md`）。
> **性质警示**：本档案是**规格要求**而非当前实现状态或验收结果——`05-状态矩阵`/`08-验收清单` 的条目均为待执行核对项，不代表 e3 或 v2.1.52 已通过。来源审计见 `../审计报告/adhoc-team-design-source-audit-2026-08-10.md`（原样归档，结论：规格来源判定成立，35→31 键数与 defaultValue 表述两项文字问题已于 P8-2 修正）。

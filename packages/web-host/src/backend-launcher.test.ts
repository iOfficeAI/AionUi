...
describe('BackendLifecycleManager.start (success path)', () => {
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    const resolveBackend = vi.fn(() => '/abs/path/poundingcore');
    ...
    try {
      ...
      expect(spawnCall[0]).toBe('/abs/path/poundingcore');
...
describe('BackendLifecycleManager crash restart', () => {
  ...
  it('logs crash restart scheduling details', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[poundingcore] child exited unexpectedly; scheduling restart', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 65303,
      restartCount: 1,
  ...
  it('logs when crash restart limit is exceeded', () => {
    ...
    expect(mgr.status).toBe('error');
    expect(errorSpy).toHaveBeenCalledWith('[poundingcore] child exited unexpectedly; restart limit exceeded', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 0,
      restartCount: 4,

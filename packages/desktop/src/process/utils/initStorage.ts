...
const STORAGE_PATH = {
  config: 'csbu-workmate-config.txt',
  chatMessage: 'csbu-workmate-chat-message.txt',
  chat: 'csbu-workmate-chat.txt',
  env: '.csbu-workmate-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[CSBU WorkMate] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[CSBU WorkMate] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[CSBU WorkMate] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('csbu-workmate.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'csbu-workmate-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'csbu-workmate-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'csbu-workmate-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'csbu-workmate-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[CSBU WorkMate] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[CSBU WorkMate:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[CSBU WorkMate] Failed to ensure assistant dirs:', error);
  }

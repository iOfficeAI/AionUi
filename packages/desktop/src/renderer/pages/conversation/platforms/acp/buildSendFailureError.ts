...
const CSBU_WORKMATE_TRANSPORT_ERROR_CODES = new Set([
...
export const buildSendFailureError = (error: unknown, message: string): AgentStreamErrorInfo => {
  ...
  if (isBackendHttpError(error) && CSBU_WORKMATE_TRANSPORT_ERROR_CODES.has(error.code)) {
    return {
      message,
      code: error.code,
      ownership: 'csbu-workmate',
  ...
  return {
    message,
    code: 'CSBU_WORKMATE_INTERNAL_ERROR',
    ownership: 'csbu-workmate',
    detail: message,
    retryable: true,
    feedback_recommended: true,

...
export const runSplitGroupMutation = async (
  ...
  if (mutation.type === 'create') {
    ...
    if (targetTag) {
      ...
      const patches: SplitGroupPatch[] = [
        { conversation_id: dragged.id, split_group: { id: targetTag.id, order: nextOrderIn(targetTag.id, census) } },
      ];
    ...
    const patches: SplitGroupPatch[] = [
      {
        ...
        split_group: { id: mutation.group_id, order: nextOrderIn(mutation.group_id, census) },
      },
    ];
    await applySplitGroupPatches(patches, previous, deps);
  ...
  if (mutation.type === 'move') {
    ...
    if (mutation.to.kind === 'group') {
      ...
      patches.push({
        conversation_id: row.id,
        split_group: { id: destination_id, order: nextOrderIn(destination_id, census) },
      });
    } else {
...
export const useSplitGroupMutations = () => {
  ...
  return { createGroup, addMember, removeMember, moveMember, leaveOwnGroup, reconcileDeleted, dissolveIfAlone };
};

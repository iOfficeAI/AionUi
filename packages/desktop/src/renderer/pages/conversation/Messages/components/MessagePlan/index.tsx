...
export const findLatestActivePlan = (messages: readonly TMessage[]): IMessagePlan | undefined => {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const message = messages[index];
    if (message.hidden || message.type !== 'plan' || message.content.entries.length === 0) continue;
    return message.content.entries.some((entry) => entry.status !== 'completed') ? message : undefined;
  }

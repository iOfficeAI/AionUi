...
import { SiderToolbar, SiderSearchEntry, SiderScheduledEntry, SiderLibraryEntry } from './SiderNav';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const handleLibraryClick = () => {
    ...
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/library')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px'>
            ...
            />
            {/* Library nav entry - cross-conversation asset browser */}
            <SiderLibraryEntry
              isMobile={isMobile}

...
const SiderDiffFlyoutTrigger: React.FC = () => {
  const { t } = useTranslation();
  const { id: conversationId } = useParams<{ id: string }>();
  const { data: conversation } = useSWR<TChatConversation | undefined>(
    conversationId ? `sider-diff-flyout.conversation.${conversationId}` : null,
  ...
  return (
    <>
      <SiderWorkspaceActionBtn
        tooltip={t('conversation.workspace.changes.expandFlyout', { defaultValue: 'Expand' })}
...
export default SiderDiffFlyoutTrigger;

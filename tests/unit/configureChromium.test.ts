...
// from running app instances, causing port conflicts. Back it up and restore.
const REAL_REGISTRY = path.join(os.homedir(), '.aicoredesktop-cdp-registry.json');
let savedRegistry: string | null = null;

function createSandbox(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'aicoredesktop-cdp-test-'));
}
...
async function loadConfigureChromium(options: SetupOptions = {}) {
  ...
  const registryPath = path.join(sandbox, '.aicoredesktop-cdp-registry.json');
...
describe('configureChromium CDP (lightweight mock + file sandbox)', () => {
  ...
  describe('dev environment isolation', () => {
    it('sets app name and userData path in dev mode', async () => {
      ...
      expect(ctx.setNameSpy).toHaveBeenCalledWith('AICoreDesktop-Dev');
      expect(ctx.setPathSpy).toHaveBeenCalledWith('userData', path.join(ctx.sandbox, 'AICoreDesktop-Dev'));
    });

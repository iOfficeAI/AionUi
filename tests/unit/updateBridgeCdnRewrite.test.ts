...
describe('updateBridge CDN URL rewriting', () => {
  ...
  it('rewrites asset.url to the CDN path and keeps GitHub URL in fallbackUrl', async () => {
    ...
    try {
      ...
      expect(macAsset?.url).toBe('https://static.command-eve.com/releases/1.9.22/AionUi-1.9.22-mac-arm64.dmg');
      expect(macAsset?.fallbackUrl).toBe(
        'https://github.com/iOfficeAI/AionUi/releases/download/v1.9.22/AionUi-1.9.22-mac-arm64.dmg'
      );

      const linuxAsset = assets.find((a: { name: string }) => a.name === 'AionUi-1.9.22-linux-amd64.deb');
      expect(linuxAsset?.url).toBe('https://static.command-eve.com/releases/1.9.22/AionUi-1.9.22-linux-amd64.deb');
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it('uses the normalized version (no v prefix) in the CDN path', async () => {
    ...
    try {
      ...
      expect(asset?.url).toMatch(/^https:\/\/static\.command-eve\.com\/releases\/1\.9\.22\//);
  ...
  it('accepts static.command-eve.com URLs for download', async () => {
    ...
    try {
      ...
      const result = await handler({
        url: 'https://static.command-eve.com/releases/1.9.22/AionUi-1.9.22-mac-arm64.dmg',

...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/halojerry/AionUi-2.0.2-dev-a3881e2/releases/tag/v1.9.22',
    ...
    assets: [
      {
        ...
        browser_download_url:
          'https://github.com/halojerry/AionUi-2.0.2-dev-a3881e2/releases/download/v1.9.22/AionUi-1.9.22-mac-arm64.dmg',
        size: 123,
      ...
      },
      {
        ...
        browser_download_url:
          'https://github.com/halojerry/AionUi-2.0.2-dev-a3881e2/releases/download/v1.9.22/AionUi-1.9.22-win-x64.exe',
        size: 456,
      ...
      },
      {
        ...
        browser_download_url:
          'https://github.com/halojerry/AionUi-2.0.2-dev-a3881e2/releases/download/v1.9.22/AionUi-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
  ...
  it('rewrites asset.url to the CDN path and keeps GitHub URL in fallbackUrl', async () => {
    ...
    try {
      ...
      expect(macAsset?.url).toBe(
        'https://yss-1256275613.cos.ap-guangzhou.myqcloud.com/releases/1.9.22/AionUi-1.9.22-mac-arm64.dmg'
      );
      ...
      expect(linuxAsset?.url).toBe(
        'https://yss-1256275613.cos.ap-guangzhou.myqcloud.com/releases/1.9.22/AionUi-1.9.22-linux-amd64.deb'
      );
    } finally {
      vi.unstubAllGlobals();
    }
  ...
  it('uses the normalized version (no v prefix) in the CDN path', async () => {
    ...
    try {
      ...
      expect(asset?.url).toMatch(
        /^https:\/\/yss-1256275613\.cos\.ap-guangzhou\.myqcloud\.com\/releases\/1\.9\.22\//
      );
...
describe('updateBridge allowlist includes CDN host', () => {
  it('accepts self-hosted COS URLs for download', async () => {
    vi.resetModules();
    vi.clearAllMocks();
    ...
    try {
      ...
      const result = await handler({
        url: 'https://yss-1256275613.cos.ap-guangzhou.myqcloud.com/releases/1.9.22/AionUi-1.9.22-mac-arm64.dmg',

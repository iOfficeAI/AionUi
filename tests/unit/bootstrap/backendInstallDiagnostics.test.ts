...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ['C:\\CSBU WorkMate\\resources', { mtimeMs: 1000, size: 0 }],
      ['C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        resourcesPath: 'C:\\CSBU WorkMate\\resources',
        checkedBundledPath: 'C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64\\aioncore.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
        execPath: 'C:\\CSBU WorkMate\\CSBU WorkMate.exe',
        isPackaged: true,
        platform: 'win32',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'aioncore.exe',
      binaryPath: 'C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64\\aioncore.exe',
      bundledDirPath: 'C:\\CSBU WorkMate\\resources\\bundled-aioncore',
      execPath: 'C:\\CSBU WorkMate\\CSBU WorkMate.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['aioncore.exe', 'managed-resources/'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\CSBU WorkMate\\resources\\bundled-aioncore\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});

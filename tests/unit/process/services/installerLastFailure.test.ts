...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'csbu-workmate-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData CSBU WorkMate directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'CSBU WorkMate.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'CSBU WorkMate.exe' }]);

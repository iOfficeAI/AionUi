...
files:
  - url: CSBU-WorkMate-2.1.45-mac-arm64.zip
    sha512: abc==
    size: 474779381
  - url: CSBU-WorkMate-2.1.45-mac-arm64.dmg
    sha512: def==
    size: 469685641
path: CSBU-WorkMate-2.1.45-mac-arm64.zip
sha512: abc==
...
describe('parseCdnManifest', () => {
  it('parses version, files and releaseDate', () => {
    ...
    expect(manifest?.files[0]).toMatchObject({ url: 'CSBU-WorkMate-2.1.45-mac-arm64.zip', size: 474779381 });
...
describe('mapCdnManifestToRelease', () => {
  it('maps files to CDN-primary assets with GitHub fallback URLs', () => {
    ...
    const release = mapCdnManifestToRelease(manifest, 'CSBU/CSBU-WorkMate');
    ...
    expect(dmg?.url).toBe('https://updates.csbu.internal/releases/2.1.45/CSBU-WorkMate-2.1.45-mac-arm64.dmg');
    expect(dmg?.fallbackUrl).toBe(
      'https://github.com/CSBU/CSBU-WorkMate/releases/download/v2.1.45/CSBU-WorkMate-2.1.45-mac-arm64.dmg'
    );

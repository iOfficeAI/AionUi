...
describe('AcpChat ad-hoc team running prop', () => {
  ...
  it('forwards isTeamRunning to AcpSendBox', async () => {
    render(<AcpChat conversation_id='conv-1' backend='claude' isTeamRunning />);

    await waitFor(() => expect(screen.getByTestId('mock-acp-sendbox')).toBeInTheDocument());
    expect(acpSendBoxMock).toHaveBeenCalledWith(expect.objectContaining({ isTeamRunning: true }));
  });
});

...
describe('TeamWarmupOverlay failure states', () => {
  it('renders a single-member failure with its simplified error', () => {
    render(
      <TeamWarmupOverlay
        phase='error'
        assistants={[assistant('l', 'leader', 'Codex'), assistant('m', 'teammate', 'CSBU WorkMate')]}
    ...
    expect(screen.getByText('Member CSBU WorkMate failed to start')).toBeInTheDocument();
  ...
  it('summarises multiple failures as a scrollable per-member list', () => {
    render(
      <TeamWarmupOverlay
        ...
        assistants={[
          ...
          assistant('a', 'teammate', 'CSBU WorkMate'),
    ...
    expect(box).toHaveTextContent('CSBU WorkMate');

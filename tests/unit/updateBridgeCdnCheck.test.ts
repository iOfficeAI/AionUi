...
const GITHUB_RELEASES = [
  {
    ...
    name: 'v2.1.45',
    body: 'changelog body',
    html_url: 'https://github.com/CSBU/CSBU-WorkMate/releases/tag/v2.1.45',
    prerelease: false,
    draft: false,
    assets: [],
  },
...
describe('update.check CDN-first', () => {
  ...
  it('reports an update from the CDN manifest and attaches GitHub notes', async () => {
    ...
    expect(res.data?.latest?.htmlUrl).toBe('https://github.com/CSBU/CSBU-WorkMate/releases/tag/v2.1.45');
    expect(res.data?.latest?.recommendedAsset?.url).toBe(
      'https://updates.csbu.internal/releases/2.1.45/CSBU-WorkMate-2.1.45-mac-arm64.dmg'
    );
  });

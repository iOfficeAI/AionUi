 ...
 * (`/Users/zhoukai/Downloads/csbu-workmate-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `csbu-workmate-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/csbu-workmate-config.txt';
...
function resolveBackendBinary(): string {
  ...
  throw new Error('aioncore binary not found (set CSBU_WORKMATE_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `csbu-workmate-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'csbu-workmate-migrate-fixture-'));

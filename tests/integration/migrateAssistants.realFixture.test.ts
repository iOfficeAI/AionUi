 ...
 * poundingcore binary using the user-provided fixtures
 ...
 *   3. Spawn a real poundingcore bound to a throw-away data-dir.
...
function resolveBackendBinary(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '../../../PoundingCore/target/debug/poundingcore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('poundingcore binary not found (set POUNDING_BACKEND_BINARY or build it)');
}

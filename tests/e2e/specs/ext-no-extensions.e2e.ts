...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'csbu-workmate-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'csbu-workmate-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      CSBU_WORKMATE_EXTENSIONS_PATH: emptyExtensionsDir,
      CSBU_WORKMATE_EXTENSION_STATES_FILE: extensionStatesFile,

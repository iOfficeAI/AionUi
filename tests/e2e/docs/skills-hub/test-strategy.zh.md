...
- ✅ SQLite 数据库独立(`csbu-workmate.db`)
- ✅ 扩展路径指向示例目录(`CSBU_WORKMATE_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.csbu-workmate/skills/',     // My Skills 存储目录
...
- E2E 框架已设置 `CSBU_WORKMATE_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'csbu-workmate-e2e-custom-'));
...
- 共享同一个 SQLite 数据库(`csbu-workmate.db`)
- 并行会导致写锁冲突 + 测试数据污染

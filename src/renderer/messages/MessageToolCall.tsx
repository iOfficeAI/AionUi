...
const MessageToolCall: React.FC<{ message: IMessageToolCall }> = ({ message }) => {
  ...
  if (message.content.name === 'google_web_search') {
    return <Alert icon={<MessageSearch theme='outline' fill={getTextColor('message.toolCall.searchIcon', 'textPrimary')} className='lh-[1]' />} content={message.content.args.query}></Alert>;
  }

...
const DingTalkConfigForm: React.FC<DingTalkConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange, onDraftConfigChange }) => {
  ...
  const statusLabel = pluginStatus?.error ? t('settings.dingtalk.statusError', 'Error') : !isTransportConnected ? t('settings.dingtalk.statusConnecting', 'Connecting...') : isRoutingHealthy === false ? t('settings.channels.routingDegraded', 'Routing Degraded') : t('settings.dingtalk.statusConnected', 'Connected');

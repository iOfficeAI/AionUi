    ...
    <div className={classNames('relative ', className)}>
      ...
      {ReactDOM.createPortal(
        <>
          ...
          {collapse ? <ExpandDownOne theme='outline' size='14' fill={getTextColor('diff.expandIcon', 'textSecondary')} className='flex items-center' onClick={() => setCollapse(false)} /> : <FoldUpOne theme='outline' size='14' fill={getTextColor('diff.collapseIcon', 'textSecondary')} className='flex items-center' onClick={() => setCollapse(true)} />}
        </>,
        operatorRef.current
      )}
    </div>

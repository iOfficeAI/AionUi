...
import { SiderToolbar, SiderSearchEntry, SiderScheduledEntry, SiderDevBrowserEntry } from './SiderNav';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const handleDevBrowserClick = () => {
    cleanupSiderTooltips();
    blurActiveElement();
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/devbrowser')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px'>
            ...
            />
            <SiderDevBrowserEntry
              isMobile={isMobile}
              isActive={pathname === '/devbrowser'}
              collapsed={collapsed}
              siderTooltipProps={siderTooltipProps}
              onClick={handleDevBrowserClick}
            />

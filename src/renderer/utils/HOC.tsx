...
type HOCComponentAndProps<
  Props extends Record<string, any> = Record<string, any>
> = [React.FC<Props>, Partial<Props>];
...
// 从右到左，对原组件进行HOC操作
const Wrapper = (
  ...HOCComponents: Array<React.FC<any> | HOCComponentAndProps>
) => {
  return <Props extends Record<string, any>>(
    Component: React.FC<Props>
  ): React.FC<Props> => {
    // 为了修复类型错误，避免 reduce 过程中类型不一致，需显式断言类型
    return HOCComponents.reverse().reduce<React.FC<Props>>(
      (Com, HOCComponent) => {

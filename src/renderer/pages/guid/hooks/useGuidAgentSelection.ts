...
export const useGuidAgentSelection = ({
  ...
  const currentConfigBackend = useMemo<AcpBackendAll | 'aionrs' | undefined>(() => {
    if (!currentConfigBackendKey) {
      return undefined;
    }
    if (currentConfigBackendKey === 'aionrs') {
      return 'aionrs';
    }
    if (Object.prototype.hasOwnProperty.call(ACP_BACKENDS_ALL, currentConfigBackendKey)) {
      return currentConfigBackendKey as AcpBackendAll;
    }
    return undefined;
    ...
    const loadPreferredConfigOptions =
      currentConfigBackendKey === 'aionrs'
        ...
        : ConfigStorage.get('acp.config').then(
            (config) => config?.[currentConfigBackendKey as AcpBackendAll]?.preferredConfigOptions || {}
          );

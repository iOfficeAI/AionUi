...
const WorkspaceGroupedHistory: React.FC<WorkspaceGroupedHistoryProps> = ({ onSessionClick, collapsed = false, batchMode = false, onBatchModeChange }) => {
  ...
  const handleConversationClick = useCallback(
    (conversation: TChatConversation) => {
      ...
      const getConversationPath = (convId: string) => {
        if (botId) {
          return `/bots/${botId}/conversation/${convId}`;
        }
      ...
      if (!customWorkspace) {
        closeAllTabs();
        void navigate(getConversationPath(conversation.id));
        if (onSessionClick) {
          onSessionClick();
        }
        return;
    ...
    [batchMode, toggleSelectedConversation, markAsRead, closeAllTabs, navigate, onSessionClick, activeTab, openTab, botId]
  );

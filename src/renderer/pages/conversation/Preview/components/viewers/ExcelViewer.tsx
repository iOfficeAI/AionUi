...
const ExcelPreview: React.FC<ExcelPreviewProps> = ({ filePath }) => {
  ...
  useEffect(() => {
    ...
    const unsubStatus = ipcBridge.excelPreview.status.on((evt) => {
      if (cancelled) return;
    ...
    const start = async () => {
      ...
      setError(null);
      try {
        const result = await ipcBridge.excelPreview.start.invoke({ filePath });
        const url = result.url;
        if (!url || ('error' in result && result.error)) {
          throw new Error((result as { error?: string }).error || t('preview.excel.watch.startFailed'));
        }
        ...
        if (!cancelled) {
          const msg = err instanceof Error ? err.message : t('preview.excel.watch.startFailed');
          setError(msg);
          setLoading(false);
        }
      }
    ...
    return () => {
      ...
      if (filePathRef.current) {
        ipcBridge.excelPreview.stop.invoke({ filePath: filePathRef.current }).catch(() => {});
      }
    };
  }, [filePath]);
      ...
      <div className='h-full w-full flex items-center justify-center bg-bg-1'>
        <div className='flex flex-col items-center gap-12px'>
          ...
          <span className='text-13px text-t-secondary'>
            {status === 'installing' ? t('preview.excel.watch.installing') : t('preview.excel.watch.loading')}
          </span>
        </div>
      </div>
    );
    ...
    return (
      <div className='h-full w-full flex items-center justify-center bg-bg-1'>
        <div className='text-center max-w-400px'>
          ...
          <div className='text-12px text-t-secondary'>{t('preview.excel.watch.installHint')}</div>
        </div>
      </div>
    );
  }
  ...
  return <iframe src={watchUrl} className='w-full h-full border-0 bg-bg-1' title='Excel Preview' />;
};

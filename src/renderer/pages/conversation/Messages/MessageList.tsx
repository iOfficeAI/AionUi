...
const MessageItem: React.FC<{ message: TMessage; highlighted?: boolean }> = React.memo(
  HOC((props) => {
    ...
    switch (message.type) {
      ...
      default:
        return (
          <div>{t("messages.unknownMessageType", { type: getUnhandledMessageType(message) })}</div>
        );
    }
  }),
  (prev, next) =>
...
const MessageList: React.FC<{ className?: string }> = () => {
  ...
  useEffect(() => {
    ...
    const targetIndex = processedList.findIndex((item) =>
      matchesTargetMessage(item, targetMessageId),
    );
    if (targetIndex === -1) {
      return;
    }
  ...
  useEffect(() => {
    const handleMessageJump = (event: Event) => {
      ...
      if (
        ...
        detail.conversationId !== conversationContext.conversationId
      )
        return;

    ...
    >
      <div className='overflow-y-auto px-24px pb-16px pr-18px max-h-[min(72vh,680px)]'>
        <Form form={form} layout='vertical'>
              ...
              {executionModeOptions.map((option) => {
                return (
                  <Radio
                    ...
                    value={option.value}
                    className={[
                      'm-0 min-w-0 text-14px text-text-2 dark:text-t-secondary',
                      isEditMode ? 'cursor-not-allowed opacity-70' : 'cursor-pointer',
                    ].join(' ')}
                  >
                    <span className='pl-4px text-14px font-medium text-text-1 dark:text-t-primary'>{option.label}</span>
                  </Radio>
                );
              })}
            </Radio.Group>
            <div className='mt-10px rounded-12px border border-solid border-[var(--color-border-2)] bg-fill-2 px-14px py-12px'>
              <p className='m-0 text-12px leading-18px text-text-2 dark:text-t-primary'>
                {selectedExecutionModeOption.description}
              </p>
              {isEditMode && (
                <p className='m-0 mt-8px text-12px leading-18px text-text-3 dark:text-t-secondary'>
          ...
          {/* Hint text */}
          {frequency !== 'manual' && (
            <p className='mt-0 mb-16px text-12px text-text-3 dark:text-t-secondary'>{t('cron.page.scheduleHint')}</p>
          )}
        </Form>
      </div>
    </ModalWrapper>

...
const initSentry = async (): Promise<void> => {
  ...
  try {
    ...
    console.warn('[AICore] Failed to initialize Sentry:', error);
    sentryMain = null;
  }
};
...
if (!gotTheLock) {
  console.warn('[AICore] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[AICore] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[AICore] Creating main window...');
  ...
  console.log(`[AICore] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[AICore] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[AICore] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[AICore] Renderer did-finish-load');
      showWindow();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[AICore] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[AICore] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[AICore] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[AICore] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[AICore] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[AICore] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[AICore] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[AICore] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[AICore] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[AICore] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[AICore] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[AICore] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[AICore] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[AICore:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[AICore] Failed to restore zoom factor:', error);
...
// Register aicoredesktop:// as the default protocol client
...
// macOS: handle aicoredesktop:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[AICore] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[AICore] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[AICore] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[AICore] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[AICore] quit (exitCode=${exitCode})`);
});

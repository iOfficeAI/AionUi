...
export default forkTask(({ data }, pipe) => {
  ...
  pipe.on('send.message', (event: { input: string; msg_id: string; files?: string[]; thinking?: ChannelThinkingLevel }, deferred) => {
    deferred.with(agent.send(event.input, event.msg_id, event.files, event.thinking));
  });

...
export async function getChannelDefaultModel(
  ...
  try {
      ...
      if (savedModel.id === GOOGLE_AUTH_PROVIDER_ID) {
        ...
        const fallback = providerList.find(
          (p) =>
            p.platform === "gemini" &&
            p.apiKey &&
            p.model?.includes(savedModel.useModel),
        );
        if (fallback) {
          return {
            ...fallback,
        ...
        const result = findProviderWithApiKey(
          savedModel.id,
          savedModel.useModel,
        );
    ...
    console.warn(
      "[SystemActions] Failed to get saved model, using default:",
      error,
    );
  }
...
export const handleSessionNew: ActionHandler = async (context) => {
  ...
  const name = getChannelConversationName(
    platform,
    convType,
    convBackend,
    channelChatId,
...
export const handleSessionStatus: ActionHandler = async (context) => {
  ...
  const session = userId
    ? sessionManager.getSession(userId, context.chatId)
    : null;
...
export const handleAgentShow: ActionHandler = async (context) => {
  ...
  if (context.platform === "dingtalk") {
    return createSuccessResponse({
      type: "text",
      text: "",
      replyMarkup: createDingTalkAgentSelectionCard(
        availableAgents,
        currentAgent,
      ),
    });
...
export const handleAgentSelect: ActionHandler = async (context, params) => {
  ...
  const availableAgents = getAvailableChannelAgents();
  const isValidAgent = availableAgents.some(
    (agent) => agent.type === newAgentType,
  );

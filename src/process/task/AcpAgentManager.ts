...
class AcpAgentManager extends BaseAgentManager<AcpAgentManagerData, AcpPermissionOption> {
  ...
  initAgent(data: AcpAgentManagerData = this.options) {
    ...
    this.bootstrap = (async () => {
      ...
      if (data.backend === "custom" && data.customAgentId) {
        ...
        const codexConfig =
          data.backend === "codex" ? await ProcessConfig.get("codex.config") : undefined;
      ...
      this.agent = new AcpAgent({
        ...
        onStreamEvent: (message) => {
          ...
          if (message.type === "agent_status") {
            ...
            const shouldDisplayStatus =
              this.isFirstMessage || status === "error" || status === "disconnected";
        ...
        onSignalEvent: async (v) => {
          ...
          if (v.type === "finish" && this.currentMsgContent && this.orchestratorBridge) {
            const delegationInput = this.orchestratorBridge.detectDelegationInText(
              this.currentMsgContent,
            );
            if (delegationInput) {
  ...
  private emitThinkingMessage(content: string, status: "thinking" | "done" = "thinking"): void {
    ...
    const duration =
      status === "done" && this.thinkingStartTime ? Date.now() - this.thinkingStartTime : undefined;
  ...
  private async runSubAgentDelegation(
    ...
    mainLog(
      "[AcpAgentManager]",
      `Sub-agent delegation ${delegationId} (${targetBackend}) collected ${outputChunks.length} chunks`,
    );

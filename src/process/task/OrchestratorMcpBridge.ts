...
export const DELEGATE_TOOL_SCHEMA = {
  ...
  inputSchema: {
    ...
    properties: {
      target_agent: {
        type: 'string',
        description: 'The backend name of the target agent. Examples: "gemini", "codex", "qwen", "claude", "opencode".',
      },
      task: {
        type: 'string',
      ...
      context: {
        type: 'string',
        description: 'Optional. Relevant context to share with the target agent (e.g., file contents, prior results).',
      },
    },
...
export class OrchestratorMcpBridge {
  ...
  async executeDelegation(
    ...
    if (!isValidAcpBackend(targetBackend)) {
      ...
      const delegation = agentOrchestrator.createDelegation(this.sourceBackend, input.target_agent, input.task, {
        parentDelegationId,
      });
    ...
    const delegation = agentOrchestrator.createDelegation(this.sourceBackend, targetBackend, input.task, {

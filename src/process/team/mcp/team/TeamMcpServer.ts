...
export class TeamMcpServer {
  ...
  private async handleSendMessage(args: Record<string, unknown>, callerSlotId?: string): Promise<string> {
    ...
    const notifyIfTargetsLead = (targetSlotId: string | undefined): void => {
      ...
      if (fromAgent?.role === 'lead') return; // lead → lead is nonsensical; ignore
    ...
    if (isShutdownApproved || isShutdownRejected) {
      ...
      // Shadowing the outer `leadAgent` is intentional here — the shutdown
      // branch was authored before the outer lookup existed; keep both in
      // lockstep by re-resolving.
      const shutdownLeadAgent = agents.find((a) => a.role === 'lead');
      const leadSlotId = shutdownLeadAgent?.slotId;

...
import { cleanupExpiredTokens, createQRToken, generateQRLoginUrlDirect as generateQRLoginUrlDirectFromService, verifyQRTokenDirect as verifyQRTokenDirectFromService } from '@/webserver/auth/qrTokenService';
...
export function generateQRLoginUrlDirect(port: number, allowRemote: boolean): { qrUrl: string; expiresAt: number } {
  return generateQRLoginUrlDirectFromService(port, allowRemote);
}
...
export async function verifyQRTokenDirect(qrToken: string, clientIP?: string): Promise<{ success: boolean; data?: { sessionToken: string; username: string }; msg?: string }> {
  return verifyQRTokenDirectFromService(qrToken, clientIP);
}
...
export function initWebuiBridge(): void {
  ...
  webui.generateQRToken.provider(async () => {
    ...
    try {
      ...
      const { token, expiresAt, qrUrl } = createQRToken(port, allowRemote);
  ...
  webui.verifyQRToken.provider(async ({ qrToken }) => {
    return verifyQRTokenDirectFromService(qrToken);
  });
